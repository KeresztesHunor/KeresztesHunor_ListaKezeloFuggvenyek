export function objektumosRendezes(lista, kulcs, novekvoSorrend)
{
    switch (typeof(lista[0][kulcs]))
    {
        case "string":
            lista.sort(novekvoSorrend ? (a, b) => a[kulcs].localeCompare(b[kulcs]) : (a, b) => b[kulcs].localeCompare(a[kulcs]));
            break;
        case "number":
            lista.sort(novekvoSorrend ? (a, b) => a[kulcs] - b[kulcs] : (a, b) => b[kulcs] - a[kulcs]);
            break;
        default:
